...
class AuthBackendTest(ZulipTestCase):
    ...
    def verify_backend(
        ...
        *,
        ...
        do_deactivate_realm(owner.realm, acting_user=owner)
...
class FetchAPIKeyTest(ZulipTestCase):
    ...
    def test_inactive_user(self) -> None:
        do_deactivate_user(self.user_profile, acting_user=self.user_profile)
        result = self.client_post(
            "/api/v1/fetch_api_key",
            dict(username=self.email, password=initial_password(self.email)),
        )
    ...
    def test_deactivated_realm(self) -> None:
        do_deactivate_realm(self.user_profile.realm, acting_user=self.user_profile)
        result = self.client_post(
            "/api/v1/fetch_api_key",
            dict(username=self.email, password=initial_password(self.email)),
        )
