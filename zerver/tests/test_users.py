...
class PermissionTest(ZulipTestCase):
    ...
    def test_updating_non_existent_user(self) -> None:
        ...
        do_change_user_role(admin, UserProfile.ROLE_REALM_ADMINISTRATOR, acting_user=admin)
    ...
    def test_owner_api(self) -> None:
        ...
        do_change_user_role(iago, UserProfile.ROLE_REALM_ADMINISTRATOR, acting_user=desdemona)
        self.login("iago")
        with tornado_redirected_to_list([]):
            result = self.client_patch(f"/json/users/{desdemona.id}", req)
    ...
    def test_access_user_by_id(self) -> None:
        ...
        hamlet = self.example_user("hamlet")
        do_deactivate_user(hamlet, acting_user=hamlet)
        with self.assertRaises(JsonableError):
    ...
    def test_change_admin_to_guest(self) -> None:
        ...
        self.login_user(iago)
        hamlet = self.example_user("hamlet")
        do_change_user_role(hamlet, UserProfile.ROLE_REALM_ADMINISTRATOR, acting_user=iago)
        self.assertFalse(hamlet.is_guest)
    ...
    def test_change_owner_to_guest(self) -> None:
        ...
        iago = self.example_user("iago")
        do_change_user_role(iago, UserProfile.ROLE_REALM_OWNER, acting_user=owner)
...
class AdminCreateUserTest(ZulipTestCase):
    def test_create_user_backend(self) -> None:
        ...
        do_change_user_role(admin, UserProfile.ROLE_MEMBER, acting_user=admin)
        result = self.client_post("/json/users", valid_params)
...
class ActivateTest(ZulipTestCase):
    def test_basics(self) -> None:
        admin = self.example_user("iago")
        user = self.example_user("hamlet")
        do_deactivate_user(user, acting_user=admin)
    ...
    def test_api_with_insufficient_permissions(self) -> None:
        ...
        do_change_user_role(non_admin, UserProfile.ROLE_MEMBER, acting_user=admin)
        self.login("othello")
