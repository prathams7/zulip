...
class TestCreateStreams(ZulipTestCase):
    def test_creating_streams(self) -> None:
        ...
        with tornado_redirected_to_list(events):
            ensure_stream(realm, "Public stream", invite_only=False, acting_user=user_profile)
        ...
        events = []
        with tornado_redirected_to_list(events):
            ensure_stream(realm, "Private stream", invite_only=True, acting_user=user_profile)
        self.assert_length(events, 1)
    ...
    def test_auto_mark_stream_created_message_as_read_for_stream_creator(self) -> None:
        ...
        announce_stream = ensure_stream(
            realm, "announce", False, "announcements here.", acting_user=user_profile
        )
...
class StreamAdminTest(ZulipTestCase):
    ...
    def test_deactivate_stream_removes_stream_from_default_stream_groups(self) -> None:
        ...
        for stream_name in ["stream1", "stream2"]:
            stream = ensure_stream(realm, stream_name, acting_user=admin)
            streams_to_keep.append(stream)

        streams_to_remove = []
        stream = ensure_stream(realm, "stream3", acting_user=admin)
    ...
    def test_change_stream_message_retention_days_requires_realm_owner(self) -> None:
        ...
        do_change_user_role(user_profile, UserProfile.ROLE_REALM_OWNER, acting_user=owner)
        result = self.client_patch(
            f"/json/streams/{stream.id}", {"message_retention_days": orjson.dumps(2).decode()}
        )
        self.assert_json_success(result)
    ...
    def delete_stream(self, stream: Stream) -> None:
        ...
        ensure_stream(realm, "!DEACTIVATED:" + active_name, acting_user=user_profile)
    ...
    def test_invite_to_stream_by_invite_period_threshold(self) -> None:
        ...
        do_change_user_role(hamlet_user, UserProfile.ROLE_REALM_ADMINISTRATOR, acting_user=admin)
        ...
        do_change_user_role(hamlet_user, UserProfile.ROLE_MEMBER, acting_user=admin)
        do_set_realm_property(hamlet_user.realm, "waiting_period_threshold", 10, acting_user=admin)
        ...
        do_set_realm_property(hamlet_user.realm, "waiting_period_threshold", 0, acting_user=admin)
        ...
        do_set_realm_property(hamlet_user.realm, "waiting_period_threshold", 20, acting_user=admin)
...
class DefaultStreamTest(ZulipTestCase):
    ...
    def test_add_and_remove_default_stream(self) -> None:
        user_profile = self.example_user("hamlet")
        realm = get_realm("zulip")
        stream = ensure_stream(realm, "Added Stream", acting_user=user_profile)
    ...
    def test_api_calls(self) -> None:
        ...
        stream = ensure_stream(user_profile.realm, stream_name, acting_user=admin)
...
class DefaultStreamGroupTest(ZulipTestCase):
    def test_create_update_and_remove_default_stream_group(self) -> None:
        ...
        for stream_name in ["stream1", "stream2", "stream3"]:
            stream = ensure_stream(realm, stream_name, acting_user=user_profile)
        ...
        for new_stream_name in new_stream_names:
            new_stream = ensure_stream(realm, new_stream_name, acting_user=user_profile)
    ...
    def test_api_calls(self) -> None:
        ...
        new_streams = []
        for new_stream_name in new_stream_names:
            new_stream = ensure_stream(realm, new_stream_name, acting_user=admin)
            new_streams.append(new_stream)
            streams.append(new_stream)
...
class SubscriptionAPITest(ZulipTestCase):
    ...
    def test_user_settings_for_subscribing_other_users(self) -> None:
        ...
        do_set_realm_property(
            realm, "create_stream_policy", Realm.POLICY_MEMBERS_ONLY, acting_user=admin
        )
        do_set_realm_property(
            realm, "invite_to_stream_policy", Realm.POLICY_ADMINS_ONLY, acting_user=admin
        )
        result = self.common_subscribe_to_streams(
            self.test_user,
            ["stream1"],
        ...
        do_set_realm_property(
            realm, "invite_to_stream_policy", Realm.POLICY_MEMBERS_ONLY, acting_user=admin
        )
        self.common_subscribe_to_streams(
            self.test_user,
            ["stream2"],
        ...
        do_set_realm_property(realm, "waiting_period_threshold", 100000, acting_user=admin)
        result = self.common_subscribe_to_streams(
            self.test_user,
            ["stream2"],
            {"principals": orjson.dumps([invitee_user_id]).decode()},
        ...
        do_set_realm_property(realm, "waiting_period_threshold", 0, acting_user=admin)
        self.common_subscribe_to_streams(
    ...
    def test_can_subscribe_other_users(self) -> None:
        ...
        admin = self.example_user("iago")
        othello = self.example_user("othello")
        do_change_user_role(othello, UserProfile.ROLE_REALM_ADMINISTRATOR, acting_user=admin)
        self.assertTrue(othello.can_subscribe_other_users())

        do_change_user_role(othello, UserProfile.ROLE_MEMBER, acting_user=admin)
        do_change_user_role(othello, UserProfile.ROLE_GUEST, acting_user=admin)
        self.assertFalse(othello.can_subscribe_other_users())

        do_change_user_role(othello, UserProfile.ROLE_MEMBER, acting_user=admin)
        do_set_realm_property(othello.realm, "waiting_period_threshold", 1000, acting_user=admin)
        do_set_realm_property(
            othello.realm,
            "invite_to_stream_policy",
            Realm.POLICY_FULL_MEMBERS_ONLY,
        ...
        do_change_user_role(othello, UserProfile.ROLE_MODERATOR, acting_user=admin)
        self.assertTrue(othello.can_subscribe_other_users())
    ...
    def test_multi_user_subscription(self) -> None:
        ...
        stream = get_stream("multi_user_stream", realm)
        with tornado_redirected_to_list(events):
            bulk_add_subscriptions(realm, [stream], [user_profile], acting_user=user_profile)
    ...
    def test_private_stream_subscription(self) -> None:
        ...
        stream_name = "private"
        stream = ensure_stream(realm, stream_name, invite_only=True, acting_user=user_profile)

        existing_user_profile = self.example_user("hamlet")
        bulk_add_subscriptions(realm, [stream], [existing_user_profile], acting_user=user_profile)
        ...
        new_stream = ensure_stream(
            realm, "private stream", invite_only=True, acting_user=user_profile
        )
        events = []
        with tornado_redirected_to_list(events):
            bulk_add_subscriptions(
                realm, [new_stream], [self.example_user("iago")], acting_user=user_profile
            )
    ...
    def test_subscribe_to_stream_post_policy_restrict_new_members_stream(self) -> None:
        ...
        do_set_realm_property(new_member.realm, "waiting_period_threshold", 10, acting_user=admin)
    ...
    def test_mark_messages_as_unread_on_unsubscribe(self) -> None:
        ...
        stream1 = ensure_stream(realm, "stream1", invite_only=False, acting_user=user)
        stream2 = ensure_stream(realm, "stream2", invite_only=False, acting_user=user)
        private = ensure_stream(realm, "private_stream", invite_only=True, acting_user=user)
...
class AccessStreamTest(ZulipTestCase):
    def test_access_stream(self) -> None:
        ...
        mit_stream = ensure_stream(mit_realm, "mit_stream", invite_only=False, acting_user=sipbtest)
        with self.assertRaisesRegex(JsonableError, "Invalid stream id"):
