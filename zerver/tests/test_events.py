...
class NormalActionsTest(BaseAction):
            ...
            with fake_backends():
                events = self.verify_action(
                    lambda: do_set_realm_authentication_methods(
                        self.user_profile.realm, auth_method_dict, acting_user=self.user_profile
                    )
                )
    ...
    def test_do_deactivate_user(self) -> None:
        bot = self.create_bot("test")
        action = lambda: do_deactivate_user(bot, acting_user=self.user_profile)
        events = self.verify_action(action, num_events=2)
    ...
    def test_do_reactivate_user(self) -> None:
        bot = self.create_bot("test")
        admin = self.example_user("iago")
        do_deactivate_user(bot, acting_user=admin)
        action = lambda: do_reactivate_user(bot, acting_user=admin)
        events = self.verify_action(action, num_events=2)
        check_realm_bot_add("events[1]", events[1])
    ...
    def test_deactivate_stream_neversubscribed(self) -> None:
        ...
        for i, include_streams in enumerate([True, False]):
            ...
            action = lambda: do_deactivate_stream(stream, acting_user=admin)
